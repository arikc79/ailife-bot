...
ЗАБОРОНЕНО: вигадувати назви інструментів. Використовуй ТІЛЬКИ реальні інструменти зі свого знання."""

TOOL_CAPABILITIES = {
    "chatgpt": "ChatGPT: Code Interpreter — аналізує таблиці/файли і будує графіки прямо в чаті; кастомні GPT під конкретні задачі; DALL-E для зображень",
    "claude": "Claude: читає документи до 200 000 слів за раз (цілу книгу або кодову базу); найточніше слідує складним інструкціям; ідеальний для юридичних/технічних текстів",
    "gemini": "Gemini: вбудований у Google Docs, Gmail, Drive — AI прямо там де ти вже працюєш; аналізує YouTube-відео за посиланням; Gemini Advanced = найсвіжіша модель Google",
    "grok": "Grok: єдиний AI з доступом до Twitter/X у реальному часі; знає що відбувається прямо зараз без затримок; відверті відповіді без корпоративної цензури",
    "perplexity": "Perplexity: кожна відповідь містить посилання на джерела — можна перевірити; шукає по актуальному вебу як Google, але дає синтез а не список посилань",
    "midjourney": "Midjourney: найвища художня якість серед усіх image-генераторів; стилізація під будь-якого художника; ідеальний для брендингу та концепт-арту",
    "ideogram": "Ideogram: єдиний генератор який точно відтворює текст всередині зображення — логотипи, банери, мем-картки без помилок у написах",
    "make": "Make (ex-Integromat): візуальні сценарії автоматизації між 1000+ сервісами; складні умови і фільтри без коду; набагато гнучкіший за Zapier для нестандартних задач",
    "zapier": "Zapier: найпростіший старт в автоматизації — зв'язує додатки за 5 хвилин; тисячі готових шаблонів; ідеальний якщо не хочеш розбиратись у складних схемах",
    "notion ai": "Notion AI: AI знає контекст твоїх нотаток і баз даних; автосаммарі зустрічей, генерація задач зі списку, переклад прямо в документі",
    "n8n": "n8n: open-source автоматизація — хостиш сам, дані нікуди не йдуть; будує складні AI-пайплайни; безкоштовний на self-hosted",
    "runway": "Runway Gen-3: генерує відео з тексту або фото за секунди; редагує конкретні об'єкти на відео не чіпаючи фон; найкращий інструмент для AI-відеоконтенту",
    "elevenlabs": "ElevenLabs: клонує твій голос за 1 хвилиною запису; найреалістичніший синтез мови; озвучення подкастів і реклами без студії",
    "suno": "Suno: генерує повноцінну пісню з вокалом і музикою за текстовим промптом; будь-який жанр за 30 секунд",
    "leonardo ai": "Leonardo AI: генерація зображень з точним контролем стилю; тренування власної моделі на своїх референсах; ідеальний для геймдизайну і брендингу",
    ...
    "nanobanana": "NanoBanana (@nanobanana_gidbot): ПОВНІСТЮ БЕЗКОШТОВНИЙ AI прямо в Telegram — без підписок, токенів і лімітів; генерує зображення з тексту або голосу за 3 секунди; редагує фото (студійний аватар із селфі, відновлення старих фото, спільне фото з зіркою, перенос в будь-яку епоху або фільм, відео-режим); також відповідає на питання, пише/перефразовує/перекладає тексти, шукає інфо в інтернеті — все в одному боті",
}
...
SUGGESTED_TOPICS = [
    ...
    "NanoBanana: як зробити студійний аватар зі звичайного селфі",
    "NanoBanana: відновлення старих фото через AI — перевірив сам",
    "NanoBanana: генерую зображення голосом прямо в Telegram",
    "NanoBanana vs Midjourney: що обрати якщо не хочеш платити за підписку",
]
...
def _find_tool_hint(topic: str) -> str:
    topic_lower = topic.lower()
    for key, desc in TOOL_CAPABILITIES.items():
        if key in topic_lower:
            return f"\nЩо унікального в цьому інструменті (використай це як основу посту):\n{desc}"
    return ""


def build_post_prompt(topic: str, style: str = None, previous: str = None) -> str:
    style_hint = f"\nСтиль: {POST_STYLES[style]}." if style else ""
    tool_hint = _find_tool_hint(topic)
    regen_hint = (
        f"\nПопередній варіант (напиши ІНШИЙ — інший гачок, інший підхід):\n{previous}"
        if previous else ""
    )
    return (
        f"Тема посту: {topic}.{style_hint}{tool_hint}{regen_hint}\n\n"
        "Правила:\n"
        ...
        "- Структура: конкретна ситуація → що зробив (інструмент + конкретна дія) → результат у цифрах → один крок для читача\n"
        "- ОБОВ'ЯЗКОВО: розкрий саме ТУ унікальну фішку інструменту яка відрізняє його від інших\n"
        "- ЗАБОРОНЕНО: писати «автоматично генерує», «допомагає з задачами», «потужний AI» — це підходить до будь-якого інструменту\n"
...
async def week_post_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if day_index == "all":
        ...
        tools_ref = "\n".join(f"- {v}" for v in TOOL_CAPABILITIES.values())
        prompt = (
            f"Згенеруй 7 окремих постів для Telegram-каналу @ailife_ua від імені Тараса.\n"
            f"Теми по днях:\n{topics_list}\n\n"
            f"Довідник реальних інструментів і їх унікальних фішок (використовуй ТІЛЬКИ їх):\n{tools_ref}\n\n"
            ...
            f"- ОБОВ'ЯЗКОВО: розкрий саме ТУ унікальну фішку інструменту яка відрізняє його від інших\n"
            f"- ЗАБОРОНЕНО: «автоматично генерує», «допомагає з задачами», «потужний AI» — це підходить до будь-якого\n"
